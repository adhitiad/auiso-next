...
export async function getRecommendedShorts(userId?: string) {
  ...
  if (!groq) return potentialShorts.slice(0, 10);
  ...
  if (userId) {
    ...
    if (history.length > 0) {
      ...
      history.forEach((h) => {
        h.video.CategoryOnVideo.forEach((c: any) =>
          watchedCategories.add(c.Category.name),
        );
      });
...
User prefers categories: ${Array.from(watchedCategories).join(", ")}.`;
  ...
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content:
            "You are a recommendation engine. Given the user's watch history and a list of available short videos, return a JSON array of the top 10 recommended video IDs from the list, sorted by relevance. Return ONLY a JSON array of strings. Do not include markdown formatting or explanation.",
        },
        {
          role: "user",
      ...
      model: "meta-llama/llama-guard-4-12b", // Fast model for quick sorting
